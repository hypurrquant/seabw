...
import { chainName, DEFAULT_CHAIN_ID } from "@seabw/core";
...
export function BasketReview() {
  ...
  useEffect(() => {
    let cancel = false;
    http<{ products: YieldProduct[] }>({ url: `/api/marketplace/yields?tier=${tier}` })
      .then((d) => { if (!cancel) setProducts(d.products ?? []); })
  ...
  const checkout = async () => {
    ...
    try {
      const data = await http<{ plan: import("@seabw/core").PipelinePlan }>({
        url: "/api/marketplace/plan",
        method: "POST",
        body: {
          tier,
      ...
      if (!data.plan) { setError("Server returned empty plan."); return; }
