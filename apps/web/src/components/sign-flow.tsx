...
import { chainName, CHAINS } from "@seabw/core";
...
export function SignFlow({ plan, onClose, onComplete }: Props) {
  ...
  const sign = async () => {
    ...
    try {
      const data = await http<{ canonicalCalldata?: typeof current.calldata; ok?: boolean; reason?: string; ruleId?: string }>({
        url: "/api/precheck",
        method: "POST",
        body: { planId: plan.planId, stepId: current.id, signerAddress: address },
      });
      if (data.canonicalCalldata) canonicalCalldata = data.canonicalCalldata;
    } catch (err) {
      setRuntime((prev) => ({
        ...prev,
        [current.id]: { status: "failed", error: `Precheck failed: ${(err as Error).message}` },
      }));
      return;
    }
    try {
