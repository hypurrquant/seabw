...
export function PlanReview() {
  ...
  const rehydrateAndSign = async (signerAddress: string) => {
    ...
    try {
      const res = await fetch("/api/plan/rehydrate", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ planId: plan.planId, signerAddress }),
      });
      const data = (await res.json()) as { plan?: PipelinePlan; error?: string };
      if (!res.ok || !data.plan) {
        setRehydrateError(data.error ?? `Server returned ${res.status}`);
        return;
      }
